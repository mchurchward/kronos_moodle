...
/**
 * Form for editing User Set HTML block instances.
 *
 * @copyright 2009 Tim Hunt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class block_usersethtml_edit_form extends block_edit_form {
    /**
    ...
    protected function specific_definition($mform) {
        ...
        $url = new moodle_url('/blocks/usersethtml/ajax.php');
        $acdivid = 'ac_input';
        $params = array(
            'datasource' => $url->out(),
            'divid' => $acdivid,
        ...
        $PAGE->requires->css('/blocks/usersethtml/styles.css');
        $PAGE->requires->yui_module('moodle-block_usersethtml-usrsetautocmp', 'M.block_usersethtml.init', array($params), null, true);

        $mform->addElement('hidden', 'config_userset', '0', array('id' => 'id_config_userset'));
        $mform->setType('config_userset', PARAM_INT);

        $mform->addElement('text', 'config_title', get_string('configtitle', 'block_usersethtml'));
        $mform->setType('config_title', PARAM_TEXT);

        $editoroptions = array('maxfiles' => EDITOR_UNLIMITED_FILES, 'noclean' => true, 'context' => $this->block->context);
        $mform->addElement('editor', 'config_text', get_string('configcontent', 'block_usersethtml'), null, $editoroptions);
        ...
        if (!empty($CFG->block_usersethtml_allowcssclasses)) {
            $mform->addElement('text', 'config_classes', get_string('configclasses', 'block_usersethtml'));
            $mform->setType('config_classes', PARAM_TEXT);
            $mform->addHelpButton('config_classes', 'configclasses', 'block_usersethtml');
        }

        // Add an auto-complete field.
        if (isset($this->block->config) && isset($this->block->config->userset) &&
                userset::exists(new field_filter('id', $this->block->config->userset))) {
            $userset = new userset($this->block->config->userset);
            $usersetinit = true;
        }

        $label = html_writer::tag('label', get_string('selecteduserset', 'block_usersethtml'), array('for' => $acdivid));
        ...
        $input = html_writer::empty_tag('input', array(
            ...
            'size' => 51,
            'maxlength' => '50',
            'placeholder' => get_string('placeholder', 'block_usersethtml'),
            'value' => $text
        ));

        $inputdescription = '';
        $configuserset = optional_param('config_userset', 0, PARAM_INT);
        $issubmitted = optional_param('_qf__block_usersethtml_edit_form', 0, PARAM_INT);
        if ($issubmitted && empty($configuserset)) {
            $inputdescription = html_writer::start_tag('p');
            $inputdescription .= html_writer::tag('span', get_string('missinguserset',  'block_usersethtml'), array('class' => 'error'));
            $inputdescription .= html_writer::end_tag('p');
        }
        $inputdescription .= html_writer::tag('p', get_string('autocompletedesc', 'block_usersethtml'));
    ...
    public function set_data($defaults) {
        if (!empty($this->block->config) && is_object($this->block->config)) {
            ...
            $text = file_prepare_draft_area($draftideditor, $this->block->context->id, 'block_usersethtml', 'content', 0, array('subdirs' => true), $currenttext);
