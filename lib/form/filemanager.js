    ...
    Y.extend(FileManagerHelper, Y.Base, {
        ...
        set_jumpto_state: function(p_extra) {
            ...
            if (!locations || locations.length < 1) {
                ...
                jumpnode.setStyle('display', 'none');
                return;
            }
        ...
        refresh_content_and_jump_options: function(p_extra) {
            // console.log('refresh_content_and_jump_options');
        ...
        on_location_item_click: function(p_sType, p_aArgs, p_extra) {
            // console.log('on_location_item_click');
        ...
        create_jumpto_elem: function(jumpnode) {
            // console.log('create_jumpto_elem');
            jumpnode.innerHTML = '';
            // Add container button for Locations menu
            var html_locations = '<input type="button" id="fmlocationbutton" name="fmlocationbutton" value="'+M.str.repository_elisfiles.jump+'">';
        ...
        print_fm_jump: function() {
            ...
            if (M.str.repository_elisfiles !== undefined && (!this.nomoodlefiles || locations) && jumpnode) {
                // this.create_jumpto_elem(jumpnode);
                this.moodlefiles = M.str.repository_elisfiles.moodle+' '+this.options.path[0].name; // TBD
                this.moodle_itemid = this.options.itemid;
            }
        },
        setup_jumpmenu_event : function(event, scope) {
            // console.log('[FM]setup_jumpmenu_event');
            var YAHOO = Y.YUI2; // ELIS-8291/ELIS-7858 - BJB130219
            // var_dump('[FM]:setup_jumpmenu_event: event = ', event);
            var fmcallback = function(e) {
                // console.log('setup_jumpmenu_event::fmcallback');
                // var_dump('fmcallback: event = ', e);
                var target = YAHOO.util.Event.getTarget(e)
                // var_dump('fmcallback: target = ', target);
                var found = false;
                var menuitems;
                var i;
                if (target.id && (menuitems = YAHOO.util.Dom.getElementsByClassName('yuimenuitemlabel'))) {
                ...
                if (!found && scope.location_button) {
                    // var_dump('fmcallback:hiding location_button = ', [] /* scope.location_button */);
                ...
                for (i in listeners) {
                    // var_dump('fmcallback::listeners['+i+'] = ', listeners[i]);
                    if (listeners[i].scope == 'Button fmlocationbutton' || listeners[i].scope == 'MenuManager') {
                        // console.log('(fmcallback)Removing listener:' +listeners[i].scope);
                        YAHOO.util.Event.removeListener(document, 'mousedown', listeners[i].fn);
                    }
                }

                // console.log('fmcallback: exit');
            };

            var listeners = YAHOO.util.Event.getListeners(document, 'mousedown');
            for (var j in listeners) {
                // var_dump('(fmcallback)listeners['+j+'] = ', listeners[j]);
                if (listeners[j].scope == 'Button fmlocationbutton') {
            ...
            // console.log('[FM]setup_jumpmenu_event: exit');
        },
        jumpto_item_keypressed: function(e, f_args) {
            // console.log('jumpto_item_keypressed');
            var YAHOO = Y.YUI2; // ELIS-8291/ELIS-7858 - BJB130219
            var scope = f_args[0];
            var shift = f_args[1];
            ...
            if (key == 9 && (e.shiftKey == shift || (shift ^ (scope.lastkey != 16)))) {
                ...
                // var_dump('(FM)jumpto_item_keypressed: hiding location_button = ', [] /* scope.location_button */);
                scope.location_button.getMenu().hide();
            }
            scope.lastkey = key;
        },
        update_location_menu: function(menudata) {
            // console.log('update_location_menu');
            var YAHOO = Y.YUI2; // ELIS-8291/ELIS-7858 - BJB130219
            var jumpnode;
            ...
            this.location_button = new YAHOO.widget.Button("fmlocationbutton",
                     { name: "fm_locations_menu",
                       label: "<span>"+M.str.repository_elisfiles.jump+"</span>",
                       type: "menu",
                       menu: menudata,
            ...
            // Format jumpto menu items
            var children = YAHOO.util.Dom.getChildren('fmlocationbutton');
            var height = menudata.length * 35;
            if (children && children.length) {
                // Override the border-style property for the firstchild span
                children[0].style.borderStyle = 'none';
                children[0].style.width = '120px';
                children[0].style.height = height.toString()+'px';
            }

            YAHOO.util.Event.addListener('fmlocationbutton-button', 'click', this.setup_jumpmenu_event, this);

            // console.log('(FM)update_location_menu: before getMenu().show()...hide()');
            ...
            // More formatting fixes for jumpto menu
            for (var i = 0; i < menuitems.length; ++i) {
                menuitems[i].style.width = '80px';
                menuitems[i].style.height = '30px';
            }
            Y.one('#fmlocationbutton').removeClass('yui-button'); // BJB140103: Fix jumpto button to appear as 'drop-down'
            // BJB140103: set menu width & height
            var outtermenu = YAHOO.util.Dom.getElementsByClassName('elis_fm-jumpto-menu');
            if (outtermenu) {
                outtermenu[0].style.width = '120px';
                outtermenu[0].style.height = height.toString()+'px';
            }
            var bd = YAHOO.util.Dom.getElementsByClassName('bd');
            if (bd) {
                bd[0].style.width = '120px';
                bd[0].style.height = height.toString()+'px';
                bd[0].style.border = 'none';
                bd[0].style.background = 'transparent';
            }
        },
        // End RL EDIT
            ...
            selectnode.one('.fp-file-download').on('click', function(e) {
                ...
                if (this.selectui.fileinfo.type != 'folder') {
                    ...
                    if (this.selectui.fileinfo.original &&
                        (starturl = this.selectui.fileinfo.original.indexOf('/repository/elisfiles/openfile.php')) > 0) {
                        srcurl = '..'+this.selectui.fileinfo.original.substr(starturl);
                    }
                    if (srcurl.indexOf('/repository/elisfiles/openfile.php') > 0) {
