...
class userset extends data_object_with_custom_fields {
    ...
    public static function unassign_user_from_track($usettrkassoc, $clusterid, $userid) {
        ...
        foreach ($usettrkassoc as $usettrk) {
            ...
            $data = local_elisprogram_usrtrk_unassign($userid, $usettrk->trackid);

            if (empty($data[0]) && empty($data[1])) {
                continue;
            }

            $trkprogram[$usettrk->trackid] = $data[0];
            $trkcls[$usettrk->trackid] = $data[1];
        }
...
function cluster_assign_to_user($clusterid, $userid, $autoenrol=true, $leader=false) {
    ...
    $eventdata['other']['clusterid'] = $clusterid;
