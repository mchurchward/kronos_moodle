...
$.fn.deepsight_action_usersettrack_assignedit = function(options) {
    ...
    this.render_action = function() {
        ...
        var checkboxid = 'deepsight_action_usersettrack_assigned_'+elementid;

        // Determine state of the autoenrol checkbox.
        ...
        // Assemble form.
        var recursiveid = 'deepsight_action_usersettrack_assigned_recursive'; // TBD: bulk?
        if (opts.parentid != 'bulklist') {
            recursiveid += '_'+opts.rowdata.element_id;
        }
        var autoassocid = 'deepsight_action_usersettrack_assigned_autoassoc'; // TBD: bulk?
        if (opts.parentid != 'bulklist') {
            autoassocid += '_'+opts.rowdata.element_id;
        }
        var form = '<form>';
        form += '<input type="checkbox" class="autoenrol" name="autoenrol" id="'+checkboxid+'" '+checkboxchecked+'/>';
        form += '<label for="'+checkboxid+'">'+opts.langautoenrol+'</label>';
        form += '<br/>';
        ...
        form += '<br/>';
        form += '<input type="checkbox" class="autoassoc hidden" name="autoassoc" id="'+autoassocid+'"/>';
        form += '<label for="'+autoassocid+'" class="autoassoc hidden" name="autoassoclabel">'+opts.langautoassoc+'</label>';
        form += '</form>';
        ...
        if (opts.mode == 'edit') {
            actionpanel.find('input.recursive').bind('click', function(e) {
                   var autoassoc = main.form.find('[class~="autoassoc"]');
                   if (autoassoc) {
                      if (this.checked) {
                          autoassoc.removeClass('hidden');
                      } else {
                          autoassoc.addClass('hidden');
                      }
                   }
            });
        }
    ...
    this.precomplete_action = function(e) {
        ...
        var autoassoc = 0;
        var autoassoccheckbox = main.form.find('input.autoassoc')
        if (autoassoccheckbox.length > 0) {
            autoassoc = (autoassoccheckbox.prop('checked') == true) ? 1 : 0;
        }
        ...
        if (opts.parentid == 'bulklist' && typeof(bulkconfirmed) == 'undefined') {
            ...
            main.actiontr.find('i.elisicon-confirm')
                ...
                .bind('click', function(e) { main.complete_action(e, autoenrol, recursive, autoassoc); });
        } else {
            main.complete_action(e, autoenrol, recursive, autoassoc);
        }
    }
     ...
     * @param int autoassoc The value of the autoassoc checkbox, or 0.
     * @return bool Success status
     */
    this.complete_action = function(e, autoenrol, recursive, autoassoc) {
