 ...
 */
$.fn.deepsight_action_usersetuser_unassign = function(options) {
    this.default_opts = {
        rowdata: null,
        parent: null,
        sesskey: null,
     ...
     */
    this.render_action = function() {//deepsight_action_usersetuser_assignedit
        // KRONOSDEV-111: Adding condition check if a cascade unenrol warning message is used and substituting token with user id.
        var desc = '';
        if (opts.parentid != 'bulklist') {
            desc = opts.desc_single;
            if (opts.cascademsg == '1') {
                desc = desc.replace(opts.cascademsgtoken, opts.parent.data('id'));
            }
        } else {
            desc = opts.desc_multiple;
        }
