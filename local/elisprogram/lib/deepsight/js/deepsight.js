 ...
 *     string   savesearchurl     The script URL to send saved search requests.
 *     string   name              Name of tab.
 *     string   pagename          Page name.
 *     int      contextid         Context id of page.
 *     object   starting_searches Searches to initially load into load search drop down.
 *     object   current_search    Default search to load.
 *     boolean  can_save_searches True if user can save searches.
 *     boolean  can_load_searches True if user can load searches.
...
$.fn.deepsight_datatable = function(options) {
    ...
    this.savesearch_queue = {timeout: null, ajax: null};
    this.loadfilter_queue = {timeout: null, ajax: null};
    ...
    /**
     * @var boolean Can save searches.
     */
    this.can_save_searches = opts.can_save_searches;

    /**
     * @var boolean Can load searches.
     */
    this.can_load_searches = opts.can_load_searches;

    /**
     * @var array Initial searches.
     */
    this.starting_searches = opts.starting_searches;

    /**
     * @var object Current search.
     */
    this.current_search = opts.current_search;

    /**
     * @var string Ajax endpoint to send saved searches to.
     */
    this.savesearchurl = opts.savesearchurl;
     ...
     */
    this.render_message = function(message, fulltable) {
        if (fulltable == true) {
            main.find('tr').remove();
            main.siblings('.ds_pagelinks').remove();
            ...
            var message = $('<div class="alert-error" style="margin: 0.5rem 0">'+message+'</div>');
            main.after(message);
            setTimeout(function() {message.fadeOut(); }, 20000);
        }
    }
    ...
    this.initialize = function() {
        ...
        main.addClass('loading');
        main.render_message(opts.lang_loading, true);
        // Initial data load is now done after default search is set.
