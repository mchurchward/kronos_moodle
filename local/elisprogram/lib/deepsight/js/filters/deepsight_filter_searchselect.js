 ...
 *     bool no_search                  Disable search text box.
 *
...
$.fn.deepsight_filter_searchselect = function(options) {
     ...
     * If label is larger than 50 chars than show up to three dots.
     */
    ...
    this.set_choices = function(choices, hideselected) {
        var options = main.filterui.dropdown.children('.options');
        var optionhtml = '';
    ...
    this.render_choice = function(id, label, checked) {
        var checkedstr = (checked == true) ? 'checked="checked"' : '', temp = null, newid = id;

        if (/[^a-z0-9]/.test(id)) {
            newid = id.replace(/[^a-z0-9]/g, '_');
        }
        temp = $('#'+eleid+'_choices_'+newid);
        if (temp.length) {
            temp.remove();
        }
        var renderedhtml = '<input type="checkbox" id="'+eleid+'_choices_'+newid+'" title="'+label+'" name="'+eleid+'_choices[]" ';
        renderedhtml += 'value="'+id+'" '+checkedstr+'/>'+'<label for="'+eleid+'_choices_'+newid+'">'+label+'</label>';
    ...
    this.set_selections = function() {
        var selectionsele = main.filterui.dropdown.children('.selections');
        var selectionhtml = '';
        for (var i in main.selections) {
    ...
    this.reset_choices = function() {
        ...
        main.filterui.dropdown.children('.filter_search').children('input[type="text"]').val('');
    }
    ...
    this.dosearch = function() {
        if (main.searchval != '') {
            var mainrequestdata = {val: main.searchval, filtername: main.name};
            var requestdata = $.extend({}, opts.additionalsearchparams, mainrequestdata);
            $.ajax({
                type: 'GET',
                url: opts.dataurl,
                data: requestdata,
    ...
    this.initialize = function() {
        ...
        main.filterui.attach_dropdown();

        var srchbox = '';
        if (typeof opts.no_search == 'undefined') {
            srchbox = '<div class="filter_search">\n\
                          <span>'+opts.lang_search+'&nbsp;&nbsp;</span><input type="text"/>\n\
                      </div>\n';
        }
        main.filterui.dropdown
        ...
        if (typeof opts.initialvalue !== "undefined" && typeof opts.initialvalue === "object") {
            ...
            for (var i = 0; i < total; i++) {
                key = opts.initialvalue[i];
                label = key;
                if (typeof (opts.initialchoices[key]) !== "undefined" && typeof (opts.initialchoices[key].label) !== "undefined") {
                    label = opts.initialchoices[key].label;
                }
                main.selections[key] = {id: key, label: label};
                opts.datatable.filter_add(main.name, key);
            }
            main.updatelabel();
        }
