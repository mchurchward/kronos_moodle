...
/**
 * DeepSight Kronos Switch Filter
 * This filter allows for selection of one of multiple choices. No Searching. No Multi-Value.
 * This file was based off of deepsight_filter_switch.js with the excpetion of the addition of a
 * Filter removal button, as well as supporting ELIS widget default filter values.
 *
 * Usage:
 *     $('#element').deepsight_filter_switch_kronos_trackenrol();
 ...
 */
$.fn.deepsight_filter_switch_kronos_trackenrol = function(options) {
    this.default_opts = {
        datatable: null,
        name: null,
        initialvalue: {},
        ...
        css_filter_class: 'deepsight_filter-switch switch_kronos_trackenrol',
    ...
    this.choose = function(label, val) {
        main.filterui.html(opts.label+' '+label);
     ...
     *                          mixed  value Whatever value to use as the filter value when this is selected
     *                          string label The label of the choice.
     * @param string val Value of selection.
     * @return object The rendered DOM node.
     */
    this.render_choices = function(choices, val) {
    ...
    this.initialize = function() {
        ...
        // KRONOSDEV-110; Removed emphasis class.
        main.filterui = $('<button></button').addClass('filterui');
        main.filterui.html(opts.label+': '+opts.choices[0].label);
        main.append(main.filterui);

        main.filterui.attach_dropdown();

        // KRONOSDEV-110: initial values array has an underscore as part of the array name.
        // Initial values are passed as arrays.
        if (typeof opts.initial_value !== 'undefined' && opts.initial_value.length === 1) {
            val = opts.initial_value[0];
        }

        main.filterui.dropdown.append(main.render_choices(opts.choices, val));

        // KRONOSDV-110: Add remove button.
    ...
    /**
     * KRONOSDEV-110: Fired when removing the filter
     *
