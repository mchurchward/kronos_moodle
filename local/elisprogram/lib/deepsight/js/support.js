 ...
 *     string focusselector              CSS selector for element to focus on when dropdown is activated.
 *
...
$.fn.attach_dropdown = function(options) {
    ...
    this.initialize = function() {
        var id = '', dropdownhtml, max = $.get_max_zindex() + 1000;
        main.addClass(opts.css_activator_class);

        // Remove drop down from DOM if it was created before hand. This prevent duplicate elements in the DOM.
        if (main.prop('id')) {
            if ($('#'+main.prop('id')+'_dropdown').length) {
                $('#'+main.prop('id')+'_dropdown').remove();
            }
            id = 'id="'+main.prop('id')+'_dropdown"';
        }

        // Render dropdown.
        dropdownhtml = '<div id="'+main.prop('id')+'_dropdown" class="'+opts.css_dropdown_class+'" ';
            ...
            .mousedown(function(e) {
                if (main.hasClass(opts.css_activator_active_class) == false) {
                    ...
                    if (opts.focusid !== null) {
                        setTimeout( function () { $(opts.focusselector).focus(); }, 250);
                    }
                }
            });
    }
...
/**
 * Extend the dialog widget to move all dialogs to the top by setting the z-index.
 */
$.widget("ui.dialog", $.ui.dialog, {
    open: function () {
        var result = this._super();
        // Override default zIndex set by dialog widget.
        var $dialog = $(this.element[0]);
        var maxZ = $.get_max_zindex();

        // Set the gray background to one zIndex less than the dialog.
...
/**
 * Clean up any dynamically created elements so there is only one version in the DOM.
 *
 * @param array cleanel Array of element id's to clean up.
 */
$.remove_dynamic_elements = function (cleanel) {
    var i = 0,
        total = cleanel.length,
        temp = null;
    // Remove any existing elements including events, other wise jQuery finds first istance and not the newly created one.
