    ...
    public function starting_searches() {
        ...
        $records = $DB->get_records_sql('SELECT *
                                     ...
                                     ORDER BY isdefault DESC, id DESC
                                        LIMIT 10',
                array($this->context->id, $this->pagename));
        $results = array();
        if (!empty($records)) {
            ...
            while (!empty($parentcontext) && $parentcontext > 1) {
                ...
                if (!empty($parentcontext)) {
                    ...
                    if (!empty($query)) {
                        ...
                        $sql = 'SELECT * FROM {local_elisprogram_deepsight} WHERE contextid = ? AND pagename = ? '.$orderby.' LIMIT 10';
                        $records = $DB->get_records_sql($sql, array($parentcontext, $this->pagename));
                    }
                    foreach ($records as $item) {
