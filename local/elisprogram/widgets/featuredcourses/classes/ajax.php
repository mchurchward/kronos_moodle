...
class ajax {
    ...
    protected function get_filter(array $data) {
        ...
        $table = '\\'.$table;
        if (strpos($table, '\eliswidget_featuredcourses\datatable\\') !== 0 || !class_exists($table)) {
    ...
    protected function get_coursesetsforprogram(array $data) {
        ...
        $datatable = new \eliswidget_featuredcourses\datatable\programcourseset($DB, $this->endpoint);
    ...
    protected function get_coursesforcourseset(array $data) {
        ...
        $datatable = new \eliswidget_featuredcourses\datatable\coursesetcourse($DB, $this->endpoint);
    ...
    protected function get_coursesforprogram(array $data) {
        ...
        if (is_numeric($data['programid'])) {
            $datatable = new \eliswidget_featuredcourses\datatable\programcourse($DB, $this->endpoint);
            $datatable->set_programid($data['programid']);
        } else if ($data['programid'] === 'none') {
            require_once(\elispm::lib('data/user.class.php'));
            $euserid = \user::get_current_userid();
            $datatable = new \eliswidget_featuredcourses\datatable\nonprogramcourse($DB, $this->endpoint);
    ...
    protected function get_classesforcourse(array $data) {
        ...
        $datatable = new \eliswidget_featuredcourses\datatable\pmclass($DB, $this->endpoint);
     ...
     *
     * @param \eliswidget_featuredcourses\datatable\base $datatable The datatable that handles the list.
     * @param array $data Received data. Includes 'filters', and 'page' keys.
     * @return array Array of JSON-able response information.
     */
    protected function get_listing_response(\eliswidget_featuredcourses\datatable\base $datatable, $data) {
