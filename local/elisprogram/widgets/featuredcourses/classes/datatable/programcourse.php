...
namespace eliswidget_featuredcourses\datatable;
...
class programcourse extends course {
    ...
    protected function get_join_sql(array $filters = array()) {
        ...
        list($customfieldsql, $customfieldjoinparam) = $this->get_featured_course_join_sql();

        return [array_merge($sql, $newsql, $customfieldsql), array_merge($params, $newparams, $customfieldjoinparam)];
    }

    /**
     * This function returns a SQL join fragment for only retrieving Programs with courses that are marked as featured courses.
     * @return array Array with additional SQL joins and the parameters.
     */
    protected function get_featured_course_join_sql() {
        ...
        $sqlfragment = ' JOIN {local_eliscore_field} field ON field.shortname = ?';
        $sqlfragment .= ' JOIN {context} ctx ON ctx.instanceid = element.id AND ctx.contextlevel = ?';
        $sqlfragment .= ' JOIN {local_eliscore_fld_data_int} fld ON fld.contextid = ctx.id AND fld.fieldid = field.id AND fld.data = 1 ';
        $sqljoins = array($sqlfragment);
        $sqljoinsparams = array($featuredcoursefield, $ctxlevel);

        return array($sqljoins, $sqljoinsparams);
    }
