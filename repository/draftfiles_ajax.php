...
switch ($action) {
    ...
    case 'list':
        ...
        if ($has_elisfiles && substr($filepath, 0, 1) != '/') {
            ...
            if ($repository) {
                try {
                    $repo = new repository_elisfiles('elisfiles', context_user::instance($USER->id),
                            array('ajax' => false, 'name' => $repository->name, 'type' => 'elisfiles'));
                    if (!empty($repo)) {
                        $listing = $repo->get_listing($filepath);
                        if (!$listing) {
                            $err = new stdClass;
                            $err->error = get_string('elis_files_folder_not_found', 'repository_elisfiles');
                            die(json_encode($err));
                        }
                    ...
                    error_log('/repository/draftfile_ajax.php: list - Exception: '.$e->getMessage());
                    $repo = null;
               }
            }
            ...
            if ($data) {
                ...
                $err = new stdClass;
                $err->error = get_string('errorreadingfile', 'repository_elisfiles', get_string('pluginname', 'repository_elisfiles'));
                die(json_encode($err));
            }
        ...
        if ($has_elisfiles && substr($filepath, 0, 1) != '/') {
            ...
            if ($repository) {
                try {
                    ...
                    $repo = null;
                    error_log('/repository/draftfile_ajax.php: mkdir - Exception: '.$e->getMessage());
               }
            }
            ...
            if (!$success) {
                $return->error = get_string('errorcouldnotcreatedirectory', 'repository_elisfiles', $newdirname);
            }
        } else {
            $fs = get_file_storage();
        ...
        if ($has_elisfiles && substr($filepath, 0, 1) != '/') {
            ...
            if ($repository) {
                try {
                    ...
                    $repo = null;
                    error_log('/repository/draftfile_ajax.php:delete - Exception: '.$e->getMessage());
               }
            }
            ...
            if (!$success) {
                // Failure message
                $return->error = get_string('couldnotdeletefile', 'repository_elisfiles', $filename.' ('.$decodedpath['path'].')');
            }
    ...
    case 'updatefile':
        ...
        if ($has_elisfiles && substr($filepath, 0, 1) != '/') {
            ...
            if (empty($newfilename) && empty($newfilepath)) { // TBD
                die(json_encode((object)array('error' => get_string('couldnotupdatefile', 'repository_elisfiles'))));
            }
            ...
            if ($repository) {
                try {
                    ...
                    $repo = null;
                    error_log('/repository/draftfile_ajax.php:update - Exception: '.$e->getMessage());
               }
            }
            if (!empty($repo) && !empty($decodedfrompath['path']) &&
                ...
                if (!$repo->can_edit_decoded_path($decodedtopath)) {
                    ...
                    $error_string = get_string('couldnotupdatefile', 'repository_elisfiles');
                ...
                if ($existing->title != $newfilename &&
                    ...
                    die(json_encode((object)array('error' =>
                        get_string('couldnotrenamenode', 'repository_elisfiles', $newfilename))));

                }
                if (!empty($decodedtopath['path']) && $decodedtopath['path'] != $decodedfrompath['path'] &&
                        elis_files_move_node($decodedfrompath['path'], $decodedtopath['path']) === false) {
                    // move failed
                    die(json_encode((object)array('error' =>
                        get_string('couldnotmovenode', 'repository_elisfiles'))));
                }
                die(json_encode((object)array('filepath' => !empty($decodedtopath['path']) ? $newfilepath : $parent))); // TBD
            }
            die(json_encode((object)array('error' => get_string('couldnotupdatefile', 'repository_elisfiles'))));
        }
        ...
        if ($has_elisfiles && substr($filepath, 0, 1) != '/') {
            ...
            if ($repository) {
                try {
                    ...
                    $repo = null;
                    error_log('/repository/draftfile_ajax.php:updatedir - Exception: '.$e->getMessage());
               }
            }
            if (!empty($repo) && !empty($newdirname) && !empty($decodedtopath['path']) && !empty($decodedfrompath['path']) &&
                    ($existing = elis_files_node_properties($decodedfrompath['path'])) !== false) {

                // Permissions checking
                if (!$repo->can_edit_decoded_path($decodedtopath)) {
                    // Note: we care about the destination directory here
                    $error_string = get_string('couldnotupdatedir', 'repository_elisfiles');
                    die(json_encode((object)array('error' => $error_string)));
                }

                // debug_errorlog_obj('updatedir: existing = ', $existing);
                if ($existing->title != $newdirname && elis_files_node_rename($decodedfrompath['path'], $newdirname) === false) {
                    // rename failed
                    die(json_encode((object)array('error' =>
                        get_string('couldnotrenamenode', 'repository_elisfiles', $newdirname)))); 
                }
            ...
            die(json_encode((object)array('error' => get_string('couldnotupdatedir', 'repository_elisfiles'))));
        }
        $fs = get_file_storage();
        ...
        if ($has_elisfiles && substr($filepath, 0, 1) != '/') {
            ...
            if ($repository) {
                try {
                    ...
                    $repo = null;
                    error_log('/repository/draftfile_ajax.php: zip - Exception: '.$e->getMessage());
               }
            }
            ...
            if ($data_error) {
                ...
                // Permissions failure
                $error_string = get_string('errorzip', 'repository_elisfiles');
                die(json_encode((object)array('error' => $error_string)));
            } else if (empty($listing->list)) {
                // Empty directory
                die(json_encode(array('error' => get_string('errorzipemptydirectory', 'repository_elisfiles'))));
            }
            $files = array();
            ...
            $found_base = false;
            if (($nodeinfo = $repo->elis_files->get_info($decodedpath['path'])) && !empty($nodeinfo->title)) {
        ...
        if ($has_elisfiles && substr($filepath, 0, 1) != '/') {
            ...
            if ($repository) {
                try {
                    ...
                    $repo = null;
                    error_log('/repository/draftfile_ajax.php: unzip - Exception: '.$e->getMessage());
                    die(json_encode(false));
               }
            }
            ...
            // Permissions checking
            if (!$repo->can_edit_decoded_path($decodedpath)) {
                $error_string = get_string('errorunzip', 'repository_elisfiles');
                die(json_encode((object)array('error' => $error_string)));
            }
            ...
            $files = array();
            if (($zipdata = $repo->get_file($decodedpath['path'], $filename)) && !empty($zipdata['path']) &&
                    ($files = $zipper->extract_to_pathname($zipdata['path'], $tmpzipdir)) && !empty($files)) {
