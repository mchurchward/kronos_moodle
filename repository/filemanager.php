...
if (empty($returnurl)) {
    $returnurl = new moodle_url('/repository/filemanager.php', array('course' => $courseid));
}

$context = context_course::instance($courseid);
$usercontext = context_user::instance($USER->id);

$title = get_string('repositorycoursefiles', 'repository_elisfiles'); // TBD
...
$capabilities = array(
    ...
    'repository/elisfiles:viewowncontent',
    'repository/elisfiles:createowncontent',
    'repository/elisfiles:viewusersetcontent',
    'repository/elisfiles:createusersetcontent'
);

if (!has_any_capability($capabilities, $context)) {
    // No access to any ELIS files areas
    require_capability('repository/elisfiles:viewcoursecontent', $context);
}

// Obtain the UUID for the default browsing location
$currentpath = elis_files_get_current_path_for_course($courseid, !has_any_capability(array('repository/elisfiles:viewsitecontent',
        'repository/elisfiles:createsitecontent',
        'repository/elisfiles:viewcoursecontent',
        'repository/elisfiles:createcoursecontent'), $context));
