 ...
 *
 * @package    core
 * @subpackage repository_elisfiles
...
$currpage   = optional_param('page', '', PARAM_RAW); // What page in repository?
...
if ($context->contextlevel == CONTEXT_COURSE) {
    $pagename = get_string("pluginname",'repository_elisfiles');

    if (!$course = $DB->get_record('course', array('id' => $context->instanceid))) {
        print_error('invalidcourseid');
    }
    require_login($course, false);
    require_capability('repository/elisfiles:view',  $context);
} else {
...
case 'search':
    ...
    try {
        ...
        $search_result['repo_id'] = $repoid;

        // TODO: need a better solution
        $purl = new moodle_url($url, array('search_paging' => 1, 'action' => 'search', 'repo_id' => $repoid));
...
case 'sign':
    ...
    if ($repo->check_login()) {
        $list = $repo->get_listing($reqpath, $currpage);
        ...
        if (!empty($list['upload'])) {
            ...
            echo '<input type="hidden" name="repo_id" value="'.s($repoid).'" /><br />';
            ...
            if (!empty($list['page'])) {
                // TODO: need a better solution
                $pagingurl = new moodle_url("$CFG->httpswwwroot/repository/files.php?action=list&itemid={$itemid}&ctx_id={$contextid}&repo_id={$repoid}&course={$courseid}");
        ...
        echo '<input type="hidden" name="repo_id" value="'.s($repoid).'" />';
        $repo->print_login();
        echo '</form>';
    }
    echo $OUTPUT->footer();
...
case 'plugins':
    $params = array();
    $params['context'] = array($usercontext, context_system::instance());
