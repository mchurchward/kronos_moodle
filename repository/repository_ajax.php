...
switch ($action) {
    ...
    case 'list':
        if ($repo->check_login()) {
            ...
            // RL EDIT: BJB130215
            if (get_class($repo) == 'repository_elisfiles' && !empty($listing['path']) && !empty($listing['path'][0]['name']) &&
                    $listing['path'][0]['name'] == $repo->elis_files->get_root()->title) {
                $listing['path'][0]['name'] = get_string('repository', 'repository_elisfiles');
            }
            // End RL EDIT
    ...
    case 'download':
        ...
        $toelisfiles = (strpos($saveas_path, '/') === 0) // TBD
                ? false : file_exists($CFG->dirroot.'/repository/elisfiles/');
        // End RL EDIT
        // validate mimetype
        $mimetypes = array();
                    ...
                    if (!empty($decodedpath['path'])) {
                        ...
                        if (!$overwriteexisting) {
                            ...
                            if ($duplicateuuid = elis_files_file_exists($saveas_filename, $listing)) {
                                $existingfilename = optional_param( 'existingfilename', $saveas_filename, PARAM_FILE); // TBD
    ...
    case 'upload':
        // RL EDIT: BJB130215
        $elisfilesexists = file_exists($CFG->dirroot.'/repository/elisfiles/lib/lib.php');
        if (method_exists($repo, 'upload')) {
            $result = $repo->upload($saveas_filename, $maxbytes);
        } else if ($elisfilesexists) {
            require_once($CFG->dirroot.'/repository/elisfiles/lib/lib.php');
            $decodedpath = unserialize(base64_decode($saveas_path));
            ...
            if (empty($decodedpath['path']) ||
                ...
                if (empty($error_message)) { // then use a generic error message
                    $error_message = get_string('errorupload', 'repository_elisfiles');
                }
                $err = new stdClass;
            ...
            if (!$duplicateelisfiles && !isset($_FILES[$elname])) {
                ...
                $err = new stdClass;
                $err->error = get_string('errorupload', 'repository_elisfiles');
                die(json_encode($err));
            }
            ...
            if (!elis_files_quota_check($filesize)) {
                $err = new stdClass;
                $err->error = get_string('erroruploadquota', 'repository_elisfiles');
                die(json_encode($err));
            }
            ...
            if (empty($saveas_filename)) {
                ...
                $realfilename = !empty($newfilename) ? $newfilename : $form_upload_name;
            } else {
        ...
        if (!$result && $elisfilesexists) {
            require_once(dirname(__FILE__).'/elisfiles/lib/elis_files_logger.class.php');
            // Handle any specific error signaled during the transfer
            $logger = elis_files_logger::instance();
    ...
    case 'overwrite':
        ...
        if ($duplicateelisfiles) {
            $elis_files_exists = file_exists($CFG->dirroot.'/repository/elisfiles/lib/lib.php');
            require_once($CFG->dirroot.'/repository/elisfiles/lib/lib.php');
...
function elis_files_duplicate_event($action, $listing, $duplicateelisfiles, $existingfilename, $saveas_path, $src, $filepath, $size, $type, $duplicateuuid, $repo_id) {
    ...
    if (!$duplicateelisfiles) {
        ...
        if (!file_exists($tmpuploaddir) && !@mkdir($tmpuploaddir, 0777, true)) {
            ...
            $err = new stdClass;
            $err->error = get_string('errorcouldnotcreatedirectory', 'repository_elisfiles', $tmpuploaddir);
            die(json_encode($err));
        }
