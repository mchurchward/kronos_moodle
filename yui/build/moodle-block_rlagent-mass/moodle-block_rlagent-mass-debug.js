...
M.block_rlagent = {
    ...
    /* @var object Contains the list of filter types for the addons. */
    filter_types: ['type', 'status', 'string'],
    ...
    addons_button_init: function() {
        ...
        function install_click(e) {
            ...
            M.block_rlagent.action_dropdown_update();
        }
        ...
        function confirm_actions(e) {
                    ...
                    buttons: [
                        {
                            ...
                            label: M.util.get_string('confirm', 'block_rlagent'),
                            section: 'footer',
                            action: function(e) {
                                e.preventDefault();
                                e.target.hide();
                                Y.one('#manage-actions-modal .modal-cancel-button').set('label', M.util.get_string('close', 'block_rlagent'));
                                do_actions(e);
                            },
                        ...
                        },
                        {
                            ...
                            label: M.util.get_string('cancel', 'block_rlagent'),
                            section: 'footer',
                            action: function(e) {
                                e.preventDefault();
            ...
            Y.Object.each(M.block_rlagent.data_actions.add, function($value, $key) {
                ...
                if ((typeof M.block_rlagent.data_addons[$key].cache === 'object') && ('dependencies' in M.block_rlagent.data_addons[$key].cache)) {
                    Y.Object.each(M.block_rlagent.data_addons[$key].cache.dependencies, function($dvalue, $dkey) {
                        if (!M.block_rlagent.data_addons[$dkey].installed) {
                            M.block_rlagent.data_actions.add[$dkey] = $dkey;
                            $dependency = {'source': $key, 'target': $dkey};
                            $report += '<ul>'+M.util.get_string('dependency_will_be_added', 'block_rlagent', $dependency);
                        }
                    });
                }
            });
            ...
            Y.Object.each(M.block_rlagent.data_actions.update, function($value, $key) {
                ...
                if ((typeof M.block_rlagent.data_addons[$key].cache === 'object') && ('dependencies' in M.block_rlagent.data_addons[$key].cache)) {
                    Y.Object.each(M.block_rlagent.data_addons[$key].cache.dependencies, function($dvalue, $dkey) {
                        if (!M.block_rlagent.data_addons[$dkey].installed) {
                            M.block_rlagent.data_actions.update[$dkey] = $dkey;
                            $dependency = {'source': $key, 'target': $dkey};
                            $report += '<ul>'+M.util.get_string('dependency_will_be_added', 'block_rlagent', $dependency);
                        }
                    });
            ...
            if ($report.length <= 0) {
                $report = '<ul>'+M.util.get_string('no_dependencies', 'block_rlagent')+'</ul>';
                Y.log($report);
            }
            $report = '<h4>'+M.util.get_string('dependencies', 'block_rlagent')+'</h4>'+$report;
                ...
                var cfg = {
                    ...
                    on: {
                        success: function(id, o) {
                            ...
                            if (Object.keys(M.block_rlagent.data_actions.add).length > 0) {
                                Y.Object.each(M.block_rlagent.data_actions.add, function($value, $key) {
                                    delete M.block_rlagent.data_actions.add[$key];
                                });
                            }
                            if (Object.keys(M.block_rlagent.data_actions.remove).length > 0) {
                                Y.Object.each(M.block_rlagent.data_actions.remove, function($value, $key) {
                                    delete M.block_rlagent.data_actions.remove[$key];
                                });
                            }
                            if (Object.keys(M.block_rlagent.data_actions.update).length > 0) {
                                Y.Object.each(M.block_rlagent.data_actions.update, function($value, $key) {
                                    delete M.block_rlagent.data_actions.update[$key];
                                });
                            }
                            M.block_rlagent.action_dropdown_update();
        ...
        $plugins.delegate('click', remove_click, 'button.btn-remove');
        $plugins.delegate('click', update_click, 'button.btn-update');
            ...
            if (!M.block_rlagent.plugin_rate_node || $this !== M.block_rlagent.plugin_rate_node) {
                ...
                $starset.each(function($star) {
                    ...
                    if ($starset.indexOf($star) === $starset.size() - 1) {
                        ...
                        M.block_rlagent.addons_rating_send($addon, $rating);
                    }
                });
            } else {
    ...
    addons_write: function() {
        ...
        M.block_rlagent.addons_rate();
        // Init action buttons *after* the plugins are printed to the page.
        M.block_rlagent.addons_button_init();
    },
    ...
    filter_init: function() {
        ...
        M.block_rlagent.filter_select();
        M.block_rlagent.filter_clear();
        M.block_rlagent.filter_text();
        M.block_rlagent.filter_checkbox();
    ...
    filter_add: function($filterstring, $mode, $refine) {
        ...
        $prefix = M.block_rlagent.capitalise_firstletter($mode) + ' | ';
        ...
        if ($labelarr.indexOf($prefix + $filterstring) <= -1) {
            ...
            M.block_rlagent.filter_plugins();
            M.block_rlagent.filter_remove();
            M.block_rlagent.filter_block_show();
            return $item;
        } else {
            return false;
        }
     ...
     */
    filter_checkbox: function() {
        $input = Y.one('input#trust-filter');

        // Select of input clears default input contents.
        $input.on('change', M.block_rlagent.filter_plugins);
    },
    ...
    filter_select: function() {
        ...
        Y.all('ul#select-filters li').on('click', function(e) {
            ...
            Y.log('$refine = '+$refine);
            M.block_rlagent.filter_add($label, $mode, $refine);
        });
    },
     ...
     */
    filter_dropdown_visibility: function() {
        var $filter  = {
            'btn': Y.one('#btn-select-filter'),
            'li': Y.all('ul#select-filters li'),
            'ul': Y.one('ul#select-filters')
        };

        $filter.btn.on('click', function() {
            if ($filter.ul.getStyle('display') === 'block') {
                $filter.ul.setStyle('display', 'none');
                $filter.ul.get('parentNode').toggleClass('open');
            } else {
                $filter.ul.setStyle('display', 'block');
                $filter.ul.get('parentNode').toggleClass('open');
            }
        });
    ...
    filter_text: function() {
        ...
        function enter_key_press() {
            ...
            if ($value.length >= 1) {
                M.block_rlagent.filter_add($value, 'string', $value);
                $input.set('value', '');
            }
            M.block_rlagent.filter_block_show();
        }
    ...
    filter_clear: function() {
        ...
        $clearbutton.on('click', function() {
            ...
            M.block_rlagent.filter_plugins();
            M.block_rlagent.filter_block_hide();
        });
    },
    ...
    filter_plugins: function() {
        ...
        var $refine;
        var $type;
        var $length = M.block_rlagent.filter_types.length;
        Y.log("Filter length: "+M.block_rlagent.filter_types.length);
        for (var $i = 0; $i < $length; $i += 1) {
            $type = M.block_rlagent.filter_types[$i];
            Y.log('Setup filter '+$type);
            M.block_rlagent.data_filters[$type] = [];

            $filters.filter('[data-filter-mode="'+$type+'"]').each(function(node) {
                $refine = Y.one(node).getAttribute('data-filter-refine');
                M.block_rlagent.data_filters[$type].push($refine);
            });
        }
        Y.log(M.block_rlagent.data_filters);
        ...
        function filter_trust() {
            ...
            Y.Object.each($addons, function($addonvalue, $addonkey) {
                $hide = true;
                if (($addonvalue.source === 4) || ($addonvalue.source === 5)) {
                    $hide = false;
                }
                Y.log("Trust filter for "+$addonkey+": "+$hide);
                if ($hide) {
        ...
        function filter_type() {
            // If there are no type filters, move on to the
            // next set of filters.
            if (M.block_rlagent.data_filters.type.length <= 0) {
                filter_status();
                return false;
            }
            var $hide;
            ...
            Y.Object.each($addons, function($addonvalue, $addonkey) {
                $hide = true;
                Y.Array.each(M.block_rlagent.data_filters.type, function($value, $index) {
                    if ($addonvalue.type === $value) {
                        $hide = false;
                    }
                });
        ...
        function filter_status() {
            // If there are no status filters, move on to the
            // next set of filters.
            if (M.block_rlagent.data_filters.status.length <= 0) {
            ...
            Y.Object.each($addons, function($addonvalue, $addonkey) {
                ...
                $hide = true;
                Y.Array.each(M.block_rlagent.data_filters.status, function($value, $index) {
                    Y.log('$value: '+$value);
                    Y.log('$index: '+$index);
                    if (($value === 'installed') && $addonvalue.installed) {
                        Y.log('type matches = '+$value);
                        $hide = false;
                    } else if (($value === 'notinstalled') && !$addonvalue.installed) {
                        Y.log('type matches = '+$value);
                        $hide = false;
                    } else if (($value === 'updateable') && $addonvalue.installed && $addonvalue.upgradeable) {
                        Y.log('type matches = '+$value);
                        $hide = false;
                    }
                });
        ...
        function filter_string() {
            if (M.block_rlagent.data_filters.string.length <= 0) {
                return false;
            }
            ...
            Y.Object.each($addons, function($addonvalue, $addonkey) {
                ...
                $hide = false;
                Y.Array.each(M.block_rlagent.data_filters.string, function($value, $index) {
                    // Get lowercaps value of string
                    $stringlowcaps = $value.toLowerCase();
    ...
    update_site_init: function() {
        ...
        Y.one('#doupdate').on('click', function(e) {
            ...
            var $continue = Y.one('div.instr-continue');
            var $after = Y.one('#afterupdate');

            // Call site update function
            YUI().use("io-base", function(Y) {
                var url = M.cfg.wwwroot+'/blocks/rlagent/mass/refresh.php';
                var cfg = {
                    method: 'POST',
                    data: '',
                    on: {
    ...
    init: function() {
        ...
        if ($update) {
            ...
            M.block_rlagent.update_continue_init();
            M.block_rlagent.update_skip_init();
